...
/* 
 * Script that Spawns & Moves a Box in Gazebo (this conveyor's world has friction)
 * 
...
void ContactsCallback(const gazebo_msgs::ContactsStateConstPtr& cs)
{
  if ( cs->states.empty()){
    isBoxPresent = 0;
    //ROS_INFO("NON CONTATTO");
  }
  else
  {  
    isBoxPresent = 1;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub = nh.subscribe("knife_bumper", 1000, ContactsCallback);
  ...
  box_move_req.wrench.force.y = 1.05;
  box_move_req.duration = ros::Duration (5);
  ...
  for(int i=0; i<box_count; ++i){
    ...
    while(isBoxPresent==0) // Waiting for the spawned box to arrive
      ros::spinOnce();
  }
